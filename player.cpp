...
void  player_c::initialize( float x, float y, int id) 
{
  ...
  t_start->x1      = x + cos( tmp_winkel ) * DEF_STEPLEN + sin( tmp_winkel ) * DEF_TRWIDTH ;
  t_start->y1      = y + sin( tmp_winkel ) * DEF_STEPLEN - cos( tmp_winkel ) * DEF_TRWIDTH ;
  t_start->x2      = x + cos( tmp_winkel ) * DEF_STEPLEN - sin( tmp_winkel ) * DEF_TRWIDTH ;
  t_start->y2      = y + sin( tmp_winkel ) * DEF_STEPLEN + cos( tmp_winkel ) * DEF_TRWIDTH ;
  t_start->status = 10 ;
...
void  player_c::render_go_step()
{
  ...
  if( isalive() ) 
  {
    ...
    step->x1     = (t_current->x1 + t_current->x2) /2.0f + cos( tmp_winkel ) * DEF_STEPLEN + sin( tmp_winkel ) * DEF_TRWIDTH ;
    step->y1     = (t_current->y1 + t_current->y2) /2.0f + sin( tmp_winkel ) * DEF_STEPLEN - cos( tmp_winkel ) * DEF_TRWIDTH ;
    step->x2     = (t_current->x1 + t_current->x2) /2.0f + cos( tmp_winkel ) * DEF_STEPLEN - sin( tmp_winkel ) * DEF_TRWIDTH ;
    step->y2     = (t_current->y1 + t_current->y2) /2.0f + sin( tmp_winkel ) * DEF_STEPLEN + cos( tmp_winkel ) * DEF_TRWIDTH ;
